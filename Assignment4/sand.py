...
def do_move(grid, x1, y1, x2, y2):
    ...
    # Transfer the value (Y COMES FIRST SINCE X is COLS and Y is ROWS, and COLS COMES FIRST BEFORE ROWS)
    grid[y2][x2] = grid[y1][x1]       # List[COLUMN_INDEX][ROW_INDEX]

    # Get rid of previous x1 y1 and replace with None
    grid[y1][x1] = None
...
def check_move(grid, x1, y1, x2, y2):
    ...
    grid_height = len(grid) # height is the same height as the first element, assumes grid is uniform
    grid_width = len(grid[0])



    # FIRST THINGS FIRST: check if point is actually in grid (prevents indexing errors later on)
    if x2 not in range(grid_width) or y2 not in range(grid_height):     # range(start_INCLUSIVE, end_EXCLUSIVE)
        return False
    ...
    # check down-right diagonal
        ...
        if grid[y2 - 1][x2] != None:    # grid[y2 - 1][x2] is the corner piece!
            return False
        else:
            return True
    ...
    # check down-left diagonal
        ...
        if grid[y2 - 1][x2] != None:    # grid[y2 - 1][x2] is the corner piece! (y is DESCENDING from top left)
            return False
        else:
            return True
...
def do_gravity(grid, x, y):
    ...
    elif check_move(grid, x, y, x, y + 1):
    ...
    # If an s is just there with no move, do nothing (returns the og grid)
    else:
        return grid
...
def do_brownian(grid, x, y, brownian):
    ...
    if grid[y][x] == 's':
        ...
        if num < brownian:
            ...
            elif coin == 1 and check_move(grid, x, y, x + 1, y):
                do_move(grid, x, y, x + 1, y)
